...
```{r echo=FALSE}
p.mean<-round(mean(totalStepDay, na.rm=TRUE))
p.median<-median(totalStepDay, na.rm=TRUE)
```

Mean and median number of steps taken each day are `r p.mean` and `r p.median`. The median is rounded off using "round()".
...
The 5-minute interval that, on average, contains the maximum number of steps is `r p.max`.
...
The number of imputing missing data, specified as "NA"", is `r p.missing`.
